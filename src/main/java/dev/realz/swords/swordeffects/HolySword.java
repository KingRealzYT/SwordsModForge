...
public class HolySword extends SwordItem {
    ...
    @Override
    public InteractionResultHolder<net.minecraft.world.item.ItemStack> use(Level worldIn, Player playerIn, InteractionHand handIn) {
        if (playerIn.getInventory().findSlotMatchingItem(new net.minecraft.world.item.ItemStack(net.minecraft.world.item.Items.ENDER_PEARL)) != -1) {
            if (!playerIn.getCooldowns().isOnCooldown(this)) {
                ...
                enderPearl.shootFromRotation(playerIn, playerIn.getXRot(), playerIn.yHeadRot, 0.0F, 1.5F, 1F);
                playerIn.level().addFreshEntity(enderPearl);
                enderPearl.setOwner(playerIn);
                net.minecraft.world.entity.player.Inventory inv = playerIn.getInventory();
                if (!playerIn.isCreative()) {
                    for (int i = 0; i < inv.getContainerSize(); i++) {
                        if (inv.getItem(i).getItem().equals(net.minecraft.world.item.Items.ENDER_PEARL)) {
                            inv.removeItem(i, 1);
                        }
                    }
                }
